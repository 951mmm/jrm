...
use syn::{
    Field, Fields, FieldsNamed, FieldsUnnamed, GenericArgument, Ident, Item, ItemEnum, ItemStruct,
    PathArguments, Token, Type, TypePath, punctuated::Punctuated,
};
...
macro_rules! debug_token_stream {
    {$expr: expr} => {
...
fn resolve_enum(item_enum: &ItemEnum) -> syn::Result<proc_macro2::TokenStream> {
    ...
    if let EnumEntry::Index(index_mata) = &attr_enum_entry {
        ...
        for variant in variants {
            ...
            let (expr, blocks) = match fields {
                Fields::Named(fields_named) => {
                    resolve_enum_fields_named(fields_named, &lit, &constructor)?
                }
                Fields::Unnamed(fields_unnamed) => {
                    let arm = resolve_enum_fields_unnamed(fields_unnamed, &lit, &constructor)?;
                    (arm, None)
                }
                Fields::Unit => (quote! {#lit => #constructor}, None),
            };
            arm_expr.push(expr);
            if let Some(blocks) = blocks {
                collection_impl_blocks.extend(blocks);
            }
        }
        ...
        let index_check = debug_token_stream! {
            quote! {println!("enum is: {}, index is: {}",stringify!(#ident), index);}
        };
        let choice_check = debug_token_stream! {
            quote! {println!("value in map is: {}", choice);}
        };
...
fn resolve_enum_fields_named(
    ...
    let (field_idents, parse_stmts, collection_impl_blocks) =
        resolve_fields_named(&fields_named.named)?;
    Ok((
        quote! {
            #lit => {
                #(#parse_stmts)*
...
fn resolve_fields_named(
    ...
    Ok((field_idents, parse_stmts, collection_impl_blocks))
}
...
mod tests {
    ...
    use crate::{class_parser::resolve_enum, derive_class_parser_inner};
